...
export default function TopbarAdmin() {
  ...
  useEffect(() => {
    const fetchUser = async () => {
      try {
        ...
        const data = res.data;
        setUser({
          name: [data?.firstName, data?.middleName, data?.lastName].filter(Boolean).join(" ") || "Admin",
          role: data?.roles?.[0]?.roleName || "Admin",
  ...
  useEffect(() => {
    ...
    const timeout = setTimeout(() => {
      axios
        .get("/api/search", { params: { q: query, role: "ADMIN" }, signal: controller.signal })
  ...
  return (
    <header className="w-full h-14 bg-background-top border-b px-3 sm:px-4 flex items-center gap-2 sm:gap-3 text-muted-foreground">
      ...
      {/* Center: Date/Time */}
      <div className="hidden xl:flex flex-1 items-center justify-center text-xs text-muted-foreground">
      ...
      <div className="ml-auto flex items-center gap-2 sm:gap-3">
        ...
        <form
          ...
          className="flex items-center gap-2 bg-background gradient-border border border-transparent rounded-md px-3 py-2 shadow-sm w-56"
        >
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <button className="relative w-10 h-10 bg-background gradient-border border border-transparent rounded-md flex items-center justify-center shadow-sm hover:bg-background/50 transition-colors">
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <div className="flex items-center gap-2 cursor-pointer border p-1 rounded-md">
              <Avatar>
                <AvatarImage src={user.avatar || "/avatar.jpg"} alt={user.name || "Admin"} />
                <AvatarFallback>{user.name?.[0] || "A"}</AvatarFallback>
              </Avatar>
              <div className="hidden md:block text-sm font-medium text-foreground">{user.name || "Admin"}</div>
            </div>
          </DropdownMenuTrigger>
          <DropdownMenuContent align="end" className="w-56">
            ...
            <DropdownMenuItem asChild>
              <a href="/admin/settings" className="cursor-pointer">Settings</a>
            </DropdownMenuItem>
            <DropdownMenuItem asChild>
              <a href="/logout" className="cursor-pointer">Logout</a>
            </DropdownMenuItem>
          </DropdownMenuContent>
        </DropdownMenu>
      </div>
